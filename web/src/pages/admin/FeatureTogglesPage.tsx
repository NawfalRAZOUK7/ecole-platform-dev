...
import { featuresService, type FeatureToggle } from '@/features/admin/api/features.api';
...
export function FeatureTogglesPage() {
  ...
  return (
    <div className="page">
      ...
      <ErrorBanner
        error={
          pageError ?? (togglesQuery.error instanceof Error ? togglesQuery.error.message : null)
        }
          ...
          >
            ...
            <label className="form-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((current) => ({ ...current, roleCodes: event.target.value }))
                }
                placeholder="ADM, TCH"
              />
            </label>
            <label className="form-field">
              ...
              <input
                className="input"
                value={draft.schoolIds}
                onChange={(event) =>
                  setDraft((current) => ({ ...current, schoolIds: event.target.value }))
                }
                ...
                onClick={() =>
                  void updateMutation.mutateAsync({
                    ...
                    payload: {
                      ...
                      enabled_role_codes: draft.roleCodes
                        .split(',')
                        .map((value) => value.trim())
                        .filter(Boolean),
                      enabled_school_ids: draft.schoolIds
                        .split(',')
                        .map((value) => value.trim())
                        .filter(Boolean),
                    },
