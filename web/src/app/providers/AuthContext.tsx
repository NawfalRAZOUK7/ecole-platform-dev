...
import { api, setAccessToken, setSchoolId, ApiClientError, type ApiError } from '@/core/api/client';
...
function mapLoginErrorToTranslationKey(error?: Pick<ApiError, 'code' | 'message'> | null): string {
  ...
  if (errCode.startsWith('ERR-OAUTH')) {
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const startOAuthLogin = useCallback(
    async (provider: 'google' | 'microsoft', schoolId: string) => {
      setState((s) => ({ ...s, isLoading: true, error: null }));
      try {
        const { getOAuthUrl } = await import('@/core/api/oauth');
        const redirectUri = `${window.location.origin}/login`;
        const { auth_url, state: oauthState } = await getOAuthUrl(provider, redirectUri);

        // Store pending state in sessionStorage for callback validation
        sessionStorage.setItem(
          'oauth_pending',
          JSON.stringify({ provider, state: oauthState, schoolId }),
        );

        window.location.href = auth_url;
      } catch (err) {
        const msg = err instanceof Error ? err.message : 'login.error.oauth';
        setState((s) => ({ ...s, isLoading: false, error: msg }));
      }
    },
    [],
  ...
  const completeOAuthLogin = useCallback(
    async (
      ...
      setState((s) => ({ ...s, isLoading: true, error: null, oauthPending: null }));
      ...
      try {
        const { exchangeOAuthCode } = await import('@/core/api/oauth');
        const redirectUri = `${window.location.origin}/login`;
