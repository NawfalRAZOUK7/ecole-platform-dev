...
import type { EventDetailProps } from '../model/calendar.types';
import type { EventRsvpStatus } from '../model/types';
...
export function EventDetail({
  ...
  const googleUrl = useMemo(
    () => buildGoogleCalendarUrl(event, i18n.language),
    [event, i18n.language],
  );
  const outlookUrl = useMemo(
    () => buildOutlookCalendarUrl(event, i18n.language),
    [event, i18n.language],
  );
  const mapUrl =
  ...
  return (
    <div className="calendar-detail-card">
      ...
      {event.description && (
        <p className="calendar-detail-card__description">{event.description}</p>
      )}
