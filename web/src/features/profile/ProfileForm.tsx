...
import type { ProfileFormProps } from './profile.types';
...
function getViewFields(userRole: string, profileData: ProfileFormProps['profileData'], t: (key: string) => string) {
  if (userRole === 'STD' && profileData?.student_profile) {
    return [
      { label: t('register.studentNumber'), value: profileData.student_profile.student_number || '—' },
  ...
  if (userRole === 'PAR' && profileData?.parent_profile) {
    return [
      { label: t('register.relationshipType'), value: profileData.parent_profile.relationship_type || '—' },
      ...
      { label: t('register.emergencyPhone'), value: profileData.parent_profile.emergency_phone || '—' },
    ];
  }
  if (userRole === 'TCH' && profileData?.teacher_profile) {
    return [
      { label: t('register.employeeId'), value: profileData.teacher_profile.employee_id || '—' },
      { label: t('register.subjectSpecialty'), value: profileData.teacher_profile.subject_specialty || '—' },
      { label: t('register.qualification'), value: profileData.teacher_profile.qualification || '—' },
...
export function ProfileForm({
  ...
  const hasProfile = Boolean(profileData?.student_profile || profileData?.parent_profile || profileData?.teacher_profile);

  return (
    <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: 16, marginTop: 16 }}>
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12 }}>{getSectionTitle(userRole, t)}</h3>
            ...
            <div className="profile-fields">
              {viewFields.map((field) => (
                <div key={field.label} className="profile-field">
                  ...
                  <span style={field.label === t('profile.rewardPoints') ? { fontWeight: 700, color: 'var(--color-primary)' } : undefined}>{field.value}</span>
                </div>
              ))}
            </div>
          ) : (
            !loading && <p style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>{t('register.noProfile')}</p>
          )}
          <button className="btn btn-secondary" onClick={() => onToggleEdit(true)} style={{ marginTop: 8 }}>
        ...
        <form onSubmit={onSubmit}>
          {editFields.map((field) => (
            <label key={field.key} className="form-field" style={{ marginBottom: 12 }}>
              ...
              {field.type === 'textarea' ? (
                <textarea className="filter-input" value={profileForm[field.key] || ''} onChange={(event) => onUpdateField(field.key, event.target.value)} style={{ width: '100%', minHeight: 60 }} />
              ) : field.type === 'select' ? (
                <select className="filter-select" value={profileForm[field.key] || ''} onChange={(event) => onUpdateField(field.key, event.target.value)} style={{ width: '100%' }}>
                  {field.options?.map((option) => (
                    <option key={option.value || option.label} value={option.value}>{option.label}</option>
                  ))}
                </select>
              ) : (
                <input type={field.type} className="filter-input" value={profileForm[field.key] || ''} onChange={(event) => onUpdateField(field.key, event.target.value)} disabled={field.disabled} style={{ width: '100%', background: field.disabled ? 'var(--color-surface)' : undefined }} />
              )}
            </label>
          ))}
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="submit" className="btn btn-primary" disabled={loading}>{loading ? t('app.loading') : t('app.save')}</button>
            <button type="button" className="btn btn-secondary" onClick={() => onToggleEdit(false)}>{t('app.cancel')}</button>
          </div>
        </form>
      )}

      {!hasProfile && !loading && !showProfileEdit && (
        <p style={{ fontSize: 14, color: 'var(--color-text-secondary)', marginTop: 12 }}>{t('register.noProfile')}</p>
      )}
    </div>
  );
}
