...
import { useAdminUserProfile, useChangePassword, useProfileChildren, useProfileData, useSaveProfileData } from './useProfile';
...
export function ProfilePage() {
  ...
  const profileLoading = profileQuery.isLoading || saveProfileMutation.isPending || adminProfileQuery.isLoading;
  ...
  return (
    <div className="page">
      <h1 className="page-title">{t(adminViewMode ? 'profile.adminView.title' : 'profile.title')}</h1>

      <div className="card profile-card">
        <AvatarUpload fullName={user.full_name} />
        <ProfileInfo children={children} childrenLoading={childrenLoading} user={user} />
        <ErrorBanner
          error={adminViewMode && adminProfileQuery.error instanceof Error ? adminProfileQuery.error.message : null}
        />
          ...
          <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: 16, marginTop: 16 }}>
            ...
            <div className="profile-fields">
              ...
              <div className="profile-field">
                ...
                <span className="role-badge">{t(`roles.${adminProfileQuery.data.role}`, adminProfileQuery.data.role)}</span>
              </div>
              <div className="profile-field">
