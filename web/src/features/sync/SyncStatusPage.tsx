...
import type { SyncCheckpoint, SyncDevice } from './sync.types';
import { useSyncCheckpoints, useSyncDevices, useSyncHealth, useSyncStatus } from './useSync';
  ...
  const deviceColumns: ColumnDef<DeviceRow>[] = useMemo(
    () => [
      ...
      { key: 'device_type', header: 'sync.deviceType' },
      {
        ...
        render: (value) => formatDate(String(value), i18n.language, { dateStyle: 'short', timeStyle: 'short' }),
      },
      {
        key: 'is_active',
        header: 'sync.active',
