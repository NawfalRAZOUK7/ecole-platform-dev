...
import type {
  ...
  SyncPushResponse,
  SyncStatus,
} from '@/entities/sync/model/types';
...
export const syncService = {
  ...
  pushChanges(deviceId: string, payload: SyncPushPayload) {
    return api.post<SyncPushResponse>(
      `/sync/push?device_id=${encodeURIComponent(deviceId)}`,
      payload,
    );
  },
  ...
  createCheckpoint(deviceId: string, payload: CreateSyncCheckpointPayload) {
    return api.post<SyncCheckpoint>(
      `/sync/checkpoint?device_id=${encodeURIComponent(deviceId)}`,
      payload,
    );
  },
