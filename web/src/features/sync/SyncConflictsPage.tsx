...
import { useResolveSyncConflict, useSyncConflicts } from './useSync';
...
export function SyncConflictsPage() {
  ...
  return (
    <div className="page">
      ...
      <ErrorBanner error={toBannerError(conflictsQuery.error ?? resolveMutation.error, t('app.error'))} />
        ...
        {(conflictsQuery.data ?? []).map((conflict) => (
          <div key={conflict.id} className="card">
            <div className="page-header page-header--split">
              ...
              <button type="button" className="btn btn-secondary" onClick={() => setSelectedConflict(conflict)}>
                {t('sync.resolve')}
              </button>
            </div>
            <div className="card-list">
              <div className="card">
                ...
                <pre style={{ whiteSpace: 'pre-wrap' }}>{JSON.stringify(conflict.client_payload, null, 2)}</pre>
              </div>
              <div className="card">
                <strong>{t('sync.keepRemote')}</strong>
                <pre style={{ whiteSpace: 'pre-wrap' }}>{JSON.stringify(conflict.server_payload, null, 2)}</pre>
              </div>
            </div>
          </div>
        ))}
      ...
      {selectedConflict ? (
        <div className="card">
          ...
          <div className="page-actions">
            <button type="button" className="btn btn-secondary" onClick={() => void handleResolve('client_wins')}>
              {t('sync.keepLocal')}
            </button>
            <button type="button" className="btn btn-secondary" onClick={() => void handleResolve('server_wins')}>
              {t('sync.keepRemote')}
            </button>
            <button type="button" className="btn btn-primary" onClick={() => void handleResolve('manual')}>
              {t('sync.manualMerge')}
            </button>
          </div>
