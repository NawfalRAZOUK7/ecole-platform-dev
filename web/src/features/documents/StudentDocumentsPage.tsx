...
import { humanSize, openSignedUrl } from './documents.utils';
import type { ChecklistItem } from './documents.service';
  ...
  async function handleUpload(category?: string) {
    ...
    try {
      await uploadMutation.mutateAsync({
        ...
        onRequestCreated: (xhr) => { uploadXhrRef.current = xhr; },
      });
      setUploadFile(null);
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th>{t('documents.checklist.uploaded')}/{t('documents.checklist.missing')}</th>
                ...
                <tbody>
                  {documents.map((doc) => (
                    <tr key={doc.id}>
                      ...
                      <td>
                        {doc.expires_at ? (
                          <span style={{ color: doc.is_expired ? 'var(--color-error)' : 'inherit' }}>
        ...
        <div className="modal-overlay" onClick={() => setUploadingFor(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <h2 style={{ marginBottom: 12 }}>
              {t('documents.uploadTitle')} — {t(`documents.categories.${uploadingFor}`, uploadingFor)}
            </h2>
            <div className="form-field">
              <input
                type="file"
