...
import { useCmsQuizzes } from './useCms';
...
export function QuizListView({ onCreate, onEdit }: QuizListViewProps) {
  ...
  return (
    <div className="page">
      ...
      <ErrorBanner error={quizzesQuery.error instanceof Error ? quizzesQuery.error.message : null} onDismiss={() => {}} onRetry={() => void quizzesQuery.refetch()} />
      ...
      {quizzes.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
          {quizzes.map((quiz) => (
            <div key={quiz.id} className="card" style={{ padding: 16, cursor: 'pointer' }} onClick={() => onEdit(quiz.id)}>
              <h3 style={{ margin: '0 0 4px', fontSize: 15 }}>{quiz.title}</h3>
              {quiz.description && <p style={{ margin: '0 0 8px', fontSize: 13, color: 'var(--color-text-secondary)' }}>{quiz.description}</p>}
