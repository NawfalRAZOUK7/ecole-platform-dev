...
export function CmsContentListPage() {
  ...
  return (
    <div className="page">
      ...
      <div className="filter-bar" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, marginBottom: 16 }}>
        <input
          className="filter-input"
          type="search"
          value={search}
          onChange={(event) => setSearch(event.target.value)}
          placeholder={t('cms.content.search')}
        />
        ...
        <select className="filter-select" value={level} onChange={(event) => setLevel(event.target.value)}>
          ...
          {LEVELS.map((currentLevel) => <option key={currentLevel} value={currentLevel}>{currentLevel}</option>)}
        </select>
        ...
        <select className="filter-select" value={language} onChange={(event) => setLanguage(event.target.value)}>
          ...
          {LANGUAGES.map((currentLanguage) => <option key={currentLanguage} value={currentLanguage}>{currentLanguage}</option>)}
        </select>
        <select className="filter-select" value={status} onChange={(event) => setStatus(event.target.value)}>
          ...
          {STATUSES.map((currentStatus) => <option key={currentStatus} value={currentStatus}>{t(`cms.statuses.${currentStatus}`, currentStatus)}</option>)}
        </select>
        <select className="filter-select" value={origin} onChange={(event) => setOrigin(event.target.value)}>
          <option value="">{t('cms.content.allOrigins')}</option>
          {ORIGINS.map((currentOrigin) => <option key={currentOrigin} value={currentOrigin}>{t(`cms.origins.${currentOrigin}`, currentOrigin)}</option>)}
        </select>
      </div>
        ...
        <>
              ...
              >
                ...
                <div style={{ padding: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 11 }}>
                    ...
                    {item.level_band ? <span className="badge">{item.level_band}</span> : null}
                    {item.subject ? <span className="badge">{t(`cms.subjects.${item.subject}`, item.subject)}</span> : null}
                    <span className={`badge badge--${item.status}`}>{t(`cms.statuses.${item.status}`, item.status)}</span>
                    {item.origin === 'PROMOTED' ? <span className="badge badge--promoted">{t('cms.origins.PROMOTED')}</span> : null}
                  </div>
                </div>
              </div>
            ))}
          ...
          {contentQuery.hasNextPage ? (
            <div style={{ textAlign: 'center', marginTop: 16 }}>
              <button className="btn" onClick={() => void contentQuery.fetchNextPage()} disabled={contentQuery.isFetchingNextPage}>
                {contentQuery.isFetchingNextPage ? t('app.loading') : t('cms.content.loadMore')}
              </button>
            </div>
          ) : null}
        </>
