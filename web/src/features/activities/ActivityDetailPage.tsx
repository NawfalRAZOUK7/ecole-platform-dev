...
function getParticipantRows(detailParticipants: ActivityParticipant[] | null | undefined, sessions: ActivitySession[]) {
  ...
  sessions.forEach((session) => {
    ...
    const completedSessions = (current.completed_sessions ?? 0) + (session.status === 'completed' ? 1 : 0);
    ...
    grouped.set(session.student_id, {
      ...
      average_score: scoreValues.length
        ...
        : null,
      status: session.status,
      last_activity_at: session.completed_at || session.started_at || current.last_activity_at || null,
    });
  });
...
export function ActivityDetailPage() {
  ...
  const activeSession = sessions.find((session) => ['started', 'in_progress'].includes(session.status));
  ...
  return (
    <div className="page">
      <div className="page-header page-header--split">
        <div>
          ...
          <p className="page-subtitle">
            {detailQuery.data?.objective || detailQuery.data?.pedagogical_objective || t('activities.noObjective')}
          </p>
        </div>
        <div className="page-actions">
          <Badge variant="info">
            {t(`activities.types.${detailQuery.data?.activity_type || detailQuery.data?.type || 'general'}`, {
              defaultValue: detailQuery.data?.activity_type || detailQuery.data?.type || 'general',
            })}
          </Badge>
      ...
      {feedbackMessage ? <div className="attendance-banner attendance-banner--success">{feedbackMessage}</div> : null}
      ...
      <div className="stats-grid">
        ...
        <StatCard
          ...
          value={gradingSummary?.completion_rate ? `${Math.round(gradingSummary.completion_rate)}%` : '0%'}
          icon="✅"
        />
      </div>
      ...
      <div className="card">
        ...
        <p>{detailQuery.data?.instructions || detailQuery.data?.description || t('activities.noInstructions')}</p>
      </div>
            ...
            content: (
              <div className="card-list">
                <div className="card">
                  ...
                  <p>{activeSession ? t('activities.activeSessionFound') : t('activities.noActiveSession')}</p>
                  <div className="page-actions">
                    <input
                      type="number"
                    ...
                    >
                      {completeSessionMutation.isPending ? t('app.loading') : t('activities.completeSession')}
                    </button>
                  </div>
                </div>
                <div className="card">
