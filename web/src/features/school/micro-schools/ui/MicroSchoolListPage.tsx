...
import type { MicroSchool } from '../model/micro-schools.types';
import { useCreateMicroSchool, useMicroSchools } from '../model/useMicroSchools';
...
export function MicroSchoolListPage() {
  ...
  return (
    <div className="page micro-schools-page">
      ...
      <ErrorBanner
        error={toBannerError(
          microSchoolsQuery.error ?? createMicroSchoolMutation.error,
          t('app.error'),
        )}
      />
      ...
      {showCreate && (
        <FormProvider {...methods}>
          <form
            className="card micro-schools-page__form"
          ...
          >
            ...
            <FormField<CreateFormValues>
              name="capacity"
              label="microSchools.capacity"
              type="number"
            />
            <button
              type="submit"
              className="btn btn-primary"
              disabled={createMicroSchoolMutation.isPending}
            >
      ...
      <div className="micro-schools-grid">
        {filteredSchools.map((school: MicroSchool) => {
          const capacityRate =
            school.capacity === 0
              ? 0
              : Math.min(100, Math.round((school.student_count / school.capacity) * 100));
          return (
            <button
              key={school.id}
              type="button"
            ...
            >
              <div className="micro-school-card__header">
                ...
                <Badge variant={getBadgeVariant(school.status)}>
                  {t(`microSchools.${school.status}`)}
                </Badge>
              </div>
              <p>
              ...
              <p>
                {t('microSchools.students')}: {school.student_count}
              </p>
              <div className="micro-school-card__capacity">
                <div
                  className="micro-school-card__capacity-bar"
