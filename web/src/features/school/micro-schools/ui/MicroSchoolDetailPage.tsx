...
import type { MicroEnrollment, MicroPayment, MicroResource } from '../model/micro-schools.types';
import {
      ...
      },
      {
        ...
        render: (_value, row) => (
          <button
            ...
            onClick={() =>
              void unenrollMutation.mutateAsync({ microSchoolId: id, enrollmentId: row.id })
            }
          >
  ...
  return (
    <div className="page micro-school-detail-page">
      <div className="page-header page-header--split">
        <div>
          ...
          <p className="page-subtitle">
            {detailQuery.data?.location}, {detailQuery.data?.city} · {t('microSchools.capacity')}:{' '}
            {detailQuery.data?.capacity ?? 0}
          </p>
        </div>
        <button
          type="button"
          className="btn btn-primary"
          onClick={() => navigate(`/micro-schools/${id}/enroll`)}
        >
      ...
      <div className="card micro-school-detail-page__hero">
        ...
        <p>
          {t('microSchools.students')}: {detailQuery.data?.student_count ?? 0}
        </p>
      </div>
          ...
          },
          {
            ...
            content: (
              <div className="card micro-school-detail-page__progress">
                <p>
                  {t('microSchools.averageProgress')}: {progressQuery.data?.average_progress ?? 0}%
                </p>
