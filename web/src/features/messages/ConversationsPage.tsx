...
import { useConversations, useCreateConversation } from './useMessages';
import type { Conversation } from './messages.service';
    ...
    <div className="page">
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontWeight: conversation.unread_count > 0 ? 700 : 600, fontSize: 15 }}>
                      {conversation.subject || getOtherParticipant(conversation)}
                    </span>
                    {conversation.unread_count > 0 && <span className="notif-badge">{conversation.unread_count}</span>}
                  </div>
      ...
      {conversationsQuery.hasNextPage && (
        <div style={{ textAlign: 'center', marginTop: 16 }}>
          <button className="btn btn-secondary" onClick={() => void conversationsQuery.fetchNextPage()} disabled={conversationsQuery.isFetchingNextPage}>
