...
import { useGradeRubric, useRubric, useRubricResults } from '../model/useRubrics';
import type { RubricGradeEntry } from '../model/rubrics.types';
...
export function RubricGradingPage() {
  ...
  if (!rubric) {
    return (
      <div className="page">
        <ErrorBanner
          error={rubricQuery.error instanceof Error ? rubricQuery.error.message : t('app.error')}
        />
      </div>
    );
  }
    ...
    <div className="page">
      <div className="page-header page-header--split">
        ...
        <button
          ...
          onClick={() => navigate(`/rubrics/${id}/edit`)}
        >
          {t('rubrics.edit')}
        </button>
      </div>
            ...
            <tfoot>
              <tr style={{ background: 'var(--color-surface)', fontWeight: 700 }}>
                ...
                <td style={{ ...TD, textAlign: 'center', fontSize: 15 }}>
                  ...
                  {allSelected && (
                    <span
                      style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginLeft: 6 }}
                    >
                      ({pct}%)
                    </span>
                  )}
