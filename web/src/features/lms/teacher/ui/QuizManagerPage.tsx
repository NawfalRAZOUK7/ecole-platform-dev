...
import { type QuizManagerView, type TeacherQuizPayload } from '../model/teacher-quiz.types';
import {
  useCreateQuiz,
  usePublishQuiz,
  useTeacherQuizzes,
} from '@/features/lms/teacher/model/useTeacher';
import type { Quiz } from '@/features/lms/teacher/api/teacher.api';
...
export function QuizManagerPage() {
  ...
  const dismissibleError = useDismissibleError(
    useMemo(
      () =>
        toBannerError(
          quizzesQuery.error ?? createQuizMutation.error ?? publishQuizMutation.error,
          t('app.error'),
        ),
