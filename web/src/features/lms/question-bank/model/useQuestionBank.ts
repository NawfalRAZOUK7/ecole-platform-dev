...
import { questionBankService } from '../api/question-bank.api';
import type {
...
export const questionBankQueryKeys = {
  ...
  list: (params?: QuestionListParams) =>
    [...questionBankQueryKeys.all, 'list', params ?? {}] as const,
...
export function useCreateQuestion() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([queryClient.invalidateQueries({ queryKey: questionBankQueryKeys.all })]);
    },
  });
}
...
export function useImportFromQuiz() {
  ...
  return useMutation({
    mutationFn: async (quizId: string) => (await questionBankService.importFromQuiz(quizId)).data,
    onSuccess: async () => {
