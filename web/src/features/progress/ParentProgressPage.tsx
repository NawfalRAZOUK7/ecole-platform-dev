...
export function ParentProgressPage() {
  ...
  if (childrenQuery.error || !data) {
    return <ErrorBanner error={childrenQuery.error instanceof Error ? childrenQuery.error.message : t('app.error')} onRetry={() => void childrenQuery.refetch()} />;
  }
          ...
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={comparisonData}>
              ...
              {data.charts.comparison.datasets.map((dataset, index) => (
                <Bar key={dataset.label} dataKey={dataset.label} fill={BAR_COLORS[index % BAR_COLORS.length]} radius={[4, 4, 0, 0]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </div>
