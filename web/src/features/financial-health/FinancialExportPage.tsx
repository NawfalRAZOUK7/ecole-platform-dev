...
import { useFinancialDashboard, useFinancialExport, useFinancialTrends } from './useFinancialHealth';
...
function buildExcelBlob(params: {
  ...
  cashflowRows: Array<{ month: string; expectedIncome: number; expectedExpenses: number; net: number }>;
}) {
  const rows = [
...
export function FinancialExportPage() {
  ...
  }, [fromDate, toDate, trendsQuery.data?.cashflow, trendsQuery.data?.retention_metrics, trendsQuery.data?.snapshots]);
  ...
  async function handleExport() {
    if (format === 'pdf') {
      ...
      downloadBlob(
        result.blob,
        `financial-health-${fromDate || 'all'}-${toDate || 'all'}.pdf`
      );
      return;
    }
