...
import { activitiesService, type ActivityFilters } from '../api/activities.api';
    ...
    onSuccess: async (_data, variables) => {
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: activitiesQueryKeys.detail(variables.activityId),
        }),
...
export function useCompleteActivitySession() {
  ...
  return useMutation({
    mutationFn: async (variables: { activityId: string; sessionId: string; score?: number }) =>
