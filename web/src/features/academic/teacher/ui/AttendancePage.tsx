...
import type { StudentItem } from '@/features/lms/teacher/api/teacher.api';
      ...
      <form onSubmit={handleSubmit}>
        <div className="filters-bar">
          <select
            ...
            aria-label={t('teacher.attendance.selectClass')}
          ...
          <select
            className="filter-select"
            aria-label={t('teacher.attendance.selectPeriod', { defaultValue: 'Select period' })}
          ...
          <input
            ...
            aria-label={t('teacher.attendance.date', { defaultValue: 'Session date' })}
            value={sessionDate}
          ...
          <select
            className="filter-select"
            aria-label={t('teacher.attendance.slot', { defaultValue: 'Attendance slot' })}
            value={slot}
            onChange={(e) => setSlot(e.target.value)}
          >
            {SLOTS.map((item) => (
              <option key={item} value={item}>
                {t(`teacher.attendance.${item}`)}
              </option>
            ))}
          </select>
        </div>
