...
import { getSubjectColor, type TimetableGridProps } from '../model/timetable.types';
...
function TimetableSlotCard({
  ...
  return (
    <div
      ...
      style={{
        background: isCanceled ? 'var(--color-surface-error)' : getSubjectColor(slot.subject),
      }}
    >
      ...
      {isAdmin && (
        <div className="timetable-slot-actions">
          <button
            ...
            onClick={() => onEdit(slot)}
            aria-label={t('timetable.editSlot')}
          >
            ✏️
          </button>
          <button
            className="btn btn-sm btn-secondary"
            onClick={() => onException(slot)}
            aria-label={t('timetable.createException')}
          >
            ⚠️
          </button>
          <button
            ...
            aria-label={t('timetable.deleteSlot', { defaultValue: 'Delete slot' })}
          >
            🗑️
          </button>
        </div>
