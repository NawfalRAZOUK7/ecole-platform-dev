...
import { skillsService } from '../api/skills.api';
import type { SkillDimension, SkillMilestone } from '../model/skills.types';
import {
...
export function SkillPassportPage() {
  ...
  const passportSections = useMemo(() => {
    ...
    return dimensions.map((dimension) => {
      const dimensionMilestones = milestones.filter(
        (milestone) => milestone.dimension_id === dimension.id,
      );
      const achieved = dimensionMilestones.filter((milestone) =>
    ...
    passportQuery.data?.progress_items,
  ]);
        ...
        <div className="card" style={{ padding: 24 }}>
          <div className="stats-grid">
            <StatCard
              ...
              value={`${passportQuery.data.overall_score}%`}
              icon="🏅"
            />
            <StatCard
              label="skills.unlocked"
              value={passportQuery.data.unlocked_milestones}
              icon="✨"
            />
            <StatCard
              label="skills.totalMilestones"
              value={passportQuery.data.total_milestones}
              icon="🏁"
            />
            <StatCard
              ...
              <div key={section.id} className="card">
                <div className="page-header page-header--split">
                  <div>
                    <h2>{section.label}</h2>
                    <p>
                      {t('skills.achievedCount', {
                        count: section.achieved.length,
                        total: section.total,
                      })}
                    </p>
                  </div>
                  <Badge
                    variant={
                      section.achieved.length === section.total && section.total > 0
                        ? 'success'
                        : 'info'
                    }
                  >
                ...
                {section.achieved.length === 0 ? (
                  ...
                  <ul>
                    {section.achieved.map((milestone) => (
                      <li key={milestone.id}>
                        {getMilestoneLabel(milestone, i18n.language)} (
                        {t('skills.levelValue', { level: milestone.level })})
                      </li>
                    ))}
                  </ul>
                )}
