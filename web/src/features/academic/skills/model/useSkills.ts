...
} from '../model/skills.types';

export const skillsQueryKeys = {
  all: ['skills'] as const,
  dimensions: () => [...skillsQueryKeys.all, 'dimensions'] as const,
  milestones: (dimensionId?: string) =>
    [...skillsQueryKeys.all, 'milestones', dimensionId || 'all'] as const,
...
export function useCreateSkillDimension() {
  ...
  return useMutation({
    mutationFn: async (payload: CreateSkillDimensionPayload) =>
      skillsService.createDimension(payload),
    onSuccess: async () => {
...
export function useCreateSkillMilestone() {
  ...
  return useMutation({
    mutationFn: async (payload: CreateSkillMilestonePayload) =>
      skillsService.createMilestone(payload),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: skillsQueryKeys.all });
    },
  });
    ...
    onSuccess: async (_data, variables) => {
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: skillsQueryKeys.progress(variables.studentId, variables.academicYearId),
        }),
        queryClient.invalidateQueries({
          queryKey: skillsQueryKeys.passport(variables.studentId, variables.academicYearId),
        }),
