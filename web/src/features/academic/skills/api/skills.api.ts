...
} from '@/entities/academic/skills/model/types';
...
export const skillsService = {
  ...
  evaluateStudent(studentId: string, academicYearId: string) {
    return api.post<SkillEvaluation>(
      `/skills/evaluate/${studentId}?academic_year_id=${encodeURIComponent(academicYearId)}`,
    );
  },
  ...
  generatePassport(studentId: string, academicYearId: string) {
    return api.post<SkillPassport>(
      `/skills/passport/${studentId}/generate?academic_year_id=${encodeURIComponent(academicYearId)}`,
    );
  },
