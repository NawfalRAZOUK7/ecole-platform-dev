...
import type { FeedItem as FeedRecord } from '../model/types';
...
export function FeedItemCard({ item, isRead, onOpen, onMarkAsRead }: FeedItemCardProps) {
    ...
    >
      <div className="feed-item-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {!isRead ? <Badge variant="warning">{t('feed.unread')}</Badge> : null}
        </div>
      ...
      <div className="page-actions">
        ...
        {!isRead ? (
          <button type="button" className="btn btn-secondary" onClick={() => onMarkAsRead(item.id)}>
            {t('feed.markRead')}
          </button>
        ) : null}
      </div>
