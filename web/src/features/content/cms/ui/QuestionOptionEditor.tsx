...
function McqEditor({ question, onChange }: QuestionOptionEditorProps) {
  ...
  function removeOption(index: number) {
    ...
    onChange({
      ...question,
      options: nextOptions,
      correct_answer: correct.filter((item) => item !== removed.id),
    });
  }
      ...
      {options.map((option, index) => (
        ...
        >
          <label style={{ display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer' }}>
            <input
              ...
              onChange={() =>
                onChange({
                  ...question,
                  correct_answer: correct.includes(option.id)
                    ? correct.filter((item) => item !== option.id)
                    : [...correct, option.id],
                })
              }
            />
          ...
          <input
            ...
            placeholder={`${t('cms.quiz.option')} ${option.id.toUpperCase()}`}
            style={{ flex: 1 }}
          />
          {options.length > 2 && (
            <button
      ...
      {options.length < 6 && (
        <button
          ...
          onClick={() =>
            onChange({
              ...question,
              options: [...options, { id: String.fromCharCode(97 + options.length), text: '' }],
            })
          }
        >
    ...
    <div style={{ display: 'flex', gap: 16 }}>
      <label style={{ cursor: 'pointer' }}>
        <input
          type="radio"
          checked={value === true}
          onChange={() => onChange({ ...question, correct_answer: true })}
        />{' '}
        {t('cms.quiz.true')}
      </label>
      <label style={{ cursor: 'pointer' }}>
        <input
          type="radio"
          checked={value === false}
          onChange={() => onChange({ ...question, correct_answer: false })}
        />{' '}
        {t('cms.quiz.false')}
      </label>
    </div>
  ...
  return (
    <div>
        ...
        <div key={index} style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
          <input
            ...
            onChange={(event) =>
              onChange({
                ...question,
                correct_answer: answers.map((item, answerIndex) =>
                  answerIndex === index ? event.target.value : item,
                ),
              })
            }
            placeholder={index === 0 ? t('cms.quiz.correctAnswer') : t('cms.quiz.alternative')}
            style={{ flex: 1 }}
          />
          {answers.length > 1 && (
            <button
              ...
              onClick={() =>
                onChange({
                  ...question,
                  correct_answer: answers.filter((_, answerIndex) => answerIndex !== index),
                })
              }
            >
              x
      ...
      <button
        type="button"
        className="btn btn-sm"
        onClick={() => onChange({ ...question, correct_answer: [...answers, ''] })}
      >
        + {t('cms.quiz.addAlternative')}
      </button>
    </div>
  );
}
