...
import { useCmsAnalytics } from '../model/useCms';
  ...
  return (
    <div className="page">
      ...
      >
        ...
        <StatCard
          label={t('cms.analytics.totalSubmissions')}
        ...
        <StatCard
          ...
          value={
            submissionStats.avg_review_time_hours != null
              ? `${submissionStats.avg_review_time_hours}h`
              : '—'
          }
          accent
        />
      ...
      >
        <div className="card" style={{ padding: 16 }}>
          ...
          {Object.entries(contentStats.by_status).map(([key, value]) => (
            <BarRow
              key={key}
              label={t(`cms.statuses.${key}`, key)}
              value={value}
              total={contentStats.total_items}
            />
          ))}
        </div>
        ...
        <div className="card" style={{ padding: 16 }}>
          ...
          {Object.entries(submissionStats.by_status).map(([key, value]) => (
            <BarRow
              key={key}
              label={t(`cms.reviewStatuses.${key}`, key)}
              value={value}
              total={submissionStats.total_submissions}
            />
          ))}
            ...
            <table style={{ width: '100%', fontSize: 13 }}>
              ...
              <tbody>
                {submissionStats.top_contributors.map((contributor, index) => (
                  <tr
                    key={`${contributor.submitter_name}-${index}`}
    ...
    <div className="card" style={{ padding: 16, textAlign: 'center' }}>
      <div
        style={{
          fontSize: 28,
          fontWeight: 700,
          color: accent ? 'var(--color-warning)' : 'var(--color-primary)',
        }}
      >
        {value}
      </div>
