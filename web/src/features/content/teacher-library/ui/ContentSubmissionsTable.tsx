...
import { REVIEW_STATUS_OPTIONS } from '../model/content-library.types';
import type { ContentSubmissionItem } from '@/features/lms/teacher/api/teacher.api';
import { useTeacherContentSubmissions } from '@/features/lms/teacher/model/useTeacher';
...
export function ContentSubmissionsTable() {
  ...
  const dismissibleError = useDismissibleError(
    useMemo(
      () => toBannerError(submissionsQuery.error, t('app.error')),
      [submissionsQuery.error, t],
    ),
  );
              ...
              {items.map((submission) => (
                <tr key={submission.id}>
                  ...
                  <td>
                    <span
                      style={{
                        color: statusColors[submission.status] || 'inherit',
                        fontWeight: 600,
                        fontSize: 13,
                      }}
                    >
