...
import type { DocumentsOptionsPayload, DocumentsTab } from '../api/documents.api';
import { RESOURCE_TYPES } from '../model/documents.types';
import { humanSize } from '../lib/documents.utils';
...
export function DocumentUpload({
  ...
  const shouldShow =
    (activeTab !== 'resources' && canUploadDocuments) ||
    (activeTab === 'resources' && canUploadResources && resourceFormOpen);
    ...
    >
      ...
      {activeTab !== 'resources' ? (
        <div className="documents-upload-fields">
          <select
            aria-label={t('documents.categoriesLabel', { defaultValue: 'Document category' })}
            value={uploadCategory}
            onChange={(event) => onChangeUploadCategory(event.target.value)}
          >
          ...
          <input
            type="date"
            aria-label={t('documents.expiryDate', { defaultValue: 'Expiry date' })}
            value={uploadExpiry}
            onChange={(event) => onChangeUploadExpiry(event.target.value)}
          />
        </div>
      ) : (
        <div className="documents-upload-fields documents-upload-fields--resource">
          <input
            aria-label={t('documents.resources.title')}
            value={resourceTitle}
            onChange={(event) => onChangeResourceTitle(event.target.value)}
            placeholder={t('documents.resources.title')}
          />
          <input
            aria-label={t('documents.resources.subject')}
            value={resourceSubject}
            onChange={(event) => onChangeResourceSubject(event.target.value)}
          ...
          />
          <select
            aria-label={t('documents.resources.type', { defaultValue: 'Resource type' })}
            value={resourceType}
            onChange={(event) =>
              onChangeResourceType(event.target.value as (typeof RESOURCE_TYPES)[number])
            }
          >
          ...
          <input
            aria-label={t('documents.resources.tags')}
            value={resourceTags}
            onChange={(event) => onChangeResourceTags(event.target.value)}
            placeholder={t('documents.resources.tags')}
          />
          <textarea
            aria-label={t('documents.resources.description')}
            value={resourceDescription}
            onChange={(event) => onChangeResourceDescription(event.target.value)}
      ...
      {isPending && (
        <div className="documents-upload-progress" role="status" aria-live="polite">
      ...
      <div className="documents-upload-actions">
        ...
        >
          {isPending ? t('documents.uploading') : t('documents.uploadAction')}
        </button>
        {isPending && (
