...
import type { DocumentItem, ResourceItem } from '../api/documents.api';
import { humanSize, isImage, isPdf } from '../lib/documents.utils';
          ...
          <div className="documents-preview-card__content">
            ...
            {previewItem.preview_url && isImage(previewItem.mime_type) && (
              <img
                ...
                className="documents-preview-card__image"
                loading="lazy"
              />
            )}
            {previewItem.preview_url && isPdf(previewItem.mime_type) && (
              <iframe
                ...
                className="documents-preview-card__frame"
              />
            )}
            ...
            {previewItem.expires_at && (
              <p>
                ...
                {formatDate(previewItem.expires_at, i18n.language, { dateStyle: 'medium' })}
              </p>
            )}
            <button
              type="button"
              className="btn btn-primary"
              onClick={() => onDownload(previewItem.download_url)}
            >
        ...
        <div className="calendar-modal-shell" role="dialog" aria-modal="true">
          <div className="calendar-modal-card documents-resource-modal">
            <div className="calendar-modal-card__header">
              ...
              <button type="button" className="btn btn-secondary" onClick={onCloseResource}>
                {t('app.close')}
              </button>
            </div>
            {isResourceLoading ? (
              ...
              <>
                {selectedResource.preview_url &&
                  ...
                  'mime_type' in selectedResource.document &&
                ...
                <p>
                  {[selectedResource.subject, selectedResource.level].filter(Boolean).join(' · ') ||
                    '—'}
                </p>
                ...
                <p>
                  {formatDate(selectedResource.created_at, i18n.language, { dateStyle: 'medium' })}
                </p>
                {selectedResource.document && 'mime_type' in selectedResource.document && (
                  <p>
                    {selectedResource.document.mime_type} ·{' '}
                    {humanSize(selectedResource.document.size_bytes)}
                  </p>
                )}
                <div className="calendar-modal-card__actions">
                  <button
                    type="button"
                    className="btn btn-primary"
                    onClick={() => onDownload(selectedResource.download_url)}
                  >
                    {t('documents.download')}
                  </button>
                    ...
                    <div className="documents-rating-group">
                      {[1, 2, 3, 4, 5].map((rating) => (
                        <button
                          ...
                          onClick={() => onRateResource(selectedResource.id, rating)}
                        >
                          {rating}★
                        </button>
                      ))}
