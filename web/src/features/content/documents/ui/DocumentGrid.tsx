      ...
      {documents.map((item) => (
        <article key={item.id} className="documents-card">
          ...
          <div className="documents-card__body">
            ...
            <p>
              {humanSize(item.size_bytes)} · {item.mime_type}
            </p>
            <div className="documents-card__meta">
              <span className="status-badge">{t(`documents.categories.${item.category}`)}</span>
              {item.is_expired && (
                <span className="status-badge status-expired">{t('documents.expired')}</span>
              )}
              {!item.is_expired && item.is_expiring_soon && (
                <span className="status-badge status-warning">{t('documents.expiringSoon')}</span>
              )}
            </div>
            <span>
          ...
          <div className="documents-card__actions">
            {item.preview_url && (
              <button type="button" className="dropdown-link" onClick={() => onPreview(item)}>
                {t('documents.preview')}
              </button>
            )}
            <button
            ...
            {item.can_delete && (
              <button type="button" className="dropdown-link" onClick={() => onDelete(item.id)}>
                {t('documents.delete')}
              </button>
            )}
          </div>
