...
import type { DocumentItem, DocumentsTab, ResourceItem } from '../api/documents.api';
import type { DocumentsPageProps } from '../model/documents.types';
import { openSignedUrl } from '../lib/documents.utils';
...
export function DocumentsPage({ initialTab = 'mine' }: DocumentsPageProps) {
  ...
  const [resourceType, setResourceType] = useState<
    'lesson_plan' | 'worksheet' | 'presentation' | 'exam_template' | 'reference'
  >('lesson_plan');
  ...
  const documentMimeOptions = useMemo(
    () =>
      Array.from(
        new Set([...(documents ?? []), ...(studentDocuments ?? [])].map((item) => item.mime_type)),
      ).sort(),
  ...
  const loading =
    ...
    (canManageStudentDocs &&
      Boolean(selectedStudentId) &&
      (studentDocumentsQuery.isLoading || checklistQuery.isLoading));
