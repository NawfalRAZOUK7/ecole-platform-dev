...
import type { DocumentViewMode } from '../model/documents.types';
import { RESOURCE_TYPES } from '../model/documents.types';
          ...
          {activeTab !== 'resources' ? (
            <>
              <input
                ...
                onChange={(event) => onChangeDocumentSearch(event.target.value)}
                placeholder={t('documents.filters.search')}
              />
              <select
                aria-label={t('documents.filters.allCategories')}
                value={documentCategoryFilter}
                onChange={(event) => onChangeDocumentCategoryFilter(event.target.value)}
              >
              ...
              <select
                aria-label={t('documents.filters.allTypes')}
                value={documentTypeFilter}
                onChange={(event) => onChangeDocumentTypeFilter(event.target.value)}
              >
              ...
              <input
                aria-label={t('documents.filters.fromDate', { defaultValue: 'From date' })}
                type="date"
                value={documentFromDate}
                onChange={(event) => onChangeDocumentFromDate(event.target.value)}
              />
              <input
                aria-label={t('documents.filters.toDate', { defaultValue: 'To date' })}
                type="date"
                value={documentToDate}
                onChange={(event) => onChangeDocumentToDate(event.target.value)}
              />
            </>
          ) : (
            <>
              <input
                aria-label={t('documents.resources.searchPlaceholder')}
                value={resourceSearch}
                onChange={(event) => onChangeResourceSearch(event.target.value)}
                placeholder={t('documents.resources.searchPlaceholder')}
              />
              <input
                aria-label={t('documents.resources.subject')}
                value={resourceFilterSubject}
                onChange={(event) => onChangeResourceFilterSubject(event.target.value)}
                placeholder={t('documents.resources.subject')}
              />
              <input
                aria-label={t('documents.resources.level')}
                value={resourceFilterLevel}
                onChange={(event) => onChangeResourceFilterLevel(event.target.value)}
                placeholder={t('documents.resources.level')}
              />
              <select
                aria-label={t('documents.resources.allTypes')}
                value={resourceFilterType}
                onChange={(event) => onChangeResourceFilterType(event.target.value)}
              >
              ...
              <select
                aria-label={t('documents.resources.allRatings')}
                value={resourceFilterRating}
                onChange={(event) => onChangeResourceFilterRating(event.target.value)}
              >
        ...
        {activeTab !== 'resources' ? (
          <div className="documents-toolbar__actions">
            ...
            {selectedDocumentCount > 0 && (
              <>
                <button type="button" className="btn btn-secondary" onClick={onBulkDownload}>
                  {t('documents.bulk.download')}
                </button>
                <button type="button" className="btn btn-secondary" onClick={onBulkDelete}>
