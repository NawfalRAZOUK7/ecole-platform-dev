...
import { useQuizAnalytics } from './useQuizzes';
...
export function QuizAnalyticsPage() {
  ...
  const completionRate = analyticsQuery.data?.total_attempts
    ? Math.round((analyticsQuery.data.completed_attempts / analyticsQuery.data.total_attempts) * 100)
    : 0;
  ...
  const scoreDistribution = useMemo(
    ...
    [analyticsQuery.data?.average_score, analyticsQuery.data?.max_score_achieved, analyticsQuery.data?.min_score_achieved, t]
  );
  ...
  return (
    <div className="page">
      ...
      <ErrorBanner error={analyticsQuery.error instanceof Error ? analyticsQuery.error.message : null} onRetry={() => void analyticsQuery.refetch()} />

      <div className="gradebook-page__stats">
        <StatCard label="quizAnalytics.totalAttempts" value={analyticsQuery.data.total_attempts} />
        <StatCard label="quizAnalytics.completedAttempts" value={analyticsQuery.data.completed_attempts} />
