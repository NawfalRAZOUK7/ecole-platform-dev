...
import { useQuizDetail, useQuizResults } from './useQuizzes';
import type { QuizQuestion } from './quizzes.service';
...
function formatAnswer(answer: unknown, question?: QuizQuestion) {
  ...
  if (typeof answer === 'object') return Object.entries(answer as Record<string, unknown>).map(([key, value]) => `${key}: ${String(value)}`).join(', ');
...
export function QuizResultsPage() {
  ...
  const percentage = attempt.max_score > 0 && attempt.score !== null
  ...
  return (
    <div className="page">
      ...
      <ErrorBanner
        ...
        onRetry={() => void Promise.all([resultsQuery.refetch(), quizId ? quizQuery.refetch() : Promise.resolve()])}
      />

      <section className="card" style={{ padding: 20, marginBottom: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16 }}>
          <div>
            <div style={{ color: 'var(--color-text-secondary)', fontSize: 13 }}>{t('quizResults.scoreSummary')}</div>
            <strong style={{ fontSize: 28 }}>{attempt.score ?? 0}/{attempt.max_score}</strong>
          </div>
          <div>
            <div style={{ color: 'var(--color-text-secondary)', fontSize: 13 }}>{t('quizResults.percentage')}</div>
            <strong style={{ fontSize: 28 }}>{percentage !== null ? `${percentage}%` : '—'}</strong>
          </div>
          <div>
            <div style={{ color: 'var(--color-text-secondary)', fontSize: 13 }}>{t('quizResults.timeTaken')}</div>
            <strong>{getDuration(attempt.started_at, attempt.completed_at)}</strong>
          </div>
          <div>
            <div style={{ color: 'var(--color-text-secondary)', fontSize: 13 }}>{t('quizResults.completedAt')}</div>
            <strong>{attempt.completed_at ? formatDate(attempt.completed_at, i18n.language, { dateStyle: 'medium', timeStyle: 'short' }) : '—'}</strong>
          </div>
        </div>
      </section>
      ...
      {responses.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {responses.map((response, index) => {
            ...
            const statusVariant = response.is_correct === null ? 'neutral' : response.is_correct ? 'success' : 'error';
            const statusLabel = response.is_correct === null
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 14, marginBottom: 4 }}>
                  <strong>{t('quizResults.yourAnswer')}:</strong> {formatAnswer(response.student_answer, question)}
                </div>
                <div style={{ fontSize: 14, marginBottom: 4 }}>
                  <strong>{t('quizResults.correctAnswer')}:</strong> {formatAnswer(response.correct_answer, question)}
                </div>
                <div style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>
                  {t('quizResults.pointsLabel', { earned: response.points_earned ?? 0, total: response.points })}
                </div>
                {response.explanation ? (
                  <p style={{ marginBottom: 0, marginTop: 10, color: 'var(--color-text-secondary)' }}>{response.explanation}</p>
                ) : null}
              </section>
            );
          })}
