...
import { financialHealthService } from '../api/financial-health.api';
import type {
...
export const financialHealthQueryKeys = {
  ...
  cost: (academicYearId: string) =>
    [...financialHealthQueryKeys.all, 'cost', academicYearId] as const,
  snapshot: (snapshotDate?: string) =>
    [...financialHealthQueryKeys.all, 'snapshot', snapshotDate || 'latest'] as const,
...
export function useFinancialRetentionMetrics(
  params: {
    skip?: number;
    limit?: number;
  } = {},
...
export function useComputeRetentionMetric() {
  ...
  return useMutation({
    mutationFn: async (payload: ComputeRetentionPayload) =>
      financialHealthService.computeRetention(payload),
    onSuccess: async () => {
...
export function useFinancialCashflowForecasts(
  params: {
    start_month?: string;
    end_month?: string;
    skip?: number;
...
export function useComputeCashflowForecast() {
  ...
  return useMutation({
    mutationFn: async (payload: ComputeCashflowPayload) =>
      financialHealthService.computeCashflow(payload),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: financialHealthQueryKeys.all });
    },
  });
...
export function useComputeCostPerStudent() {
  ...
  return useMutation({
    mutationFn: async (payload: ComputeCostPerStudentPayload) =>
      financialHealthService.computeCostPerStudent(payload),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: financialHealthQueryKeys.all });
    },
  });
...
export function useComputeFinancialSnapshot() {
  ...
  return useMutation({
    mutationFn: async (payload: ComputeFinancialSnapshotPayload) =>
      financialHealthService.computeSnapshot(payload),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: financialHealthQueryKeys.all });
    },
  });
