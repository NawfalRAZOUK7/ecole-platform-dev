...
import type { CalendarClassOption, CalendarEventItem, CalendarView, EventFormState } from './types';

export const EVENT_TYPES = ['holiday', 'exam', 'meeting', 'excursion', 'ceremony', 'custom'] as const;
...
export function formatEventRange(event: Pick<CalendarEventItem, 'start_at' | 'end_at' | 'is_all_day'>, lang: string) {
  if (event.is_all_day) {
...
export function toEventFormState(event: CalendarEventItem | null | undefined, userRole: string, availableClasses: CalendarClassOption[]): EventFormState {
...
export function toEventPayload(form: EventFormState, userRole: string) {
  return {
    ...
    class_id: (userRole === 'TCH' || form.visibility === 'class') && form.class_id ? form.class_id : undefined,
    role_codes: form.visibility === 'role' ? form.role_codes : undefined,
    is_all_day: form.is_all_day,
    recurrence_rule: form.recurrence_frequency ? { frequency: form.recurrence_frequency, interval: Number(form.recurrence_interval || 1), until: form.recurrence_until ? new Date(form.recurrence_until).toISOString() : undefined } : undefined,
...
export function buildGoogleCalendarUrl(event: CalendarEventItem, lang: string) {
  const params = new URLSearchParams({ action: 'TEMPLATE', text: eventTitle(event, lang), dates: `${toUtcStamp(event.start_at)}/${toUtcStamp(event.end_at)}`, details: event.description || '', location: event.location || '' });
  return `https://calendar.google.com/calendar/render?${params.toString()}`;
}

export function buildOutlookCalendarUrl(event: CalendarEventItem, lang: string) {
  const params = new URLSearchParams({ path: '/calendar/action/compose', rru: 'addevent', subject: eventTitle(event, lang), startdt: event.start_at, enddt: event.end_at, body: event.description || '', location: event.location || '' });
  return `https://outlook.live.com/calendar/0/deeplink/compose?${params.toString()}`;
}

export function downloadEventIcs(event: CalendarEventItem, lang: string) {
  const ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Ecole Platform//Calendar//EN', 'BEGIN:VEVENT', `UID:${event.instance_id}@ecole-platform.ma`, `DTSTAMP:${toUtcStamp(new Date().toISOString())}`, event.is_all_day ? `DTSTART;VALUE=DATE:${toUtcStamp(event.start_at).slice(0, 8)}` : `DTSTART:${toUtcStamp(event.start_at)}`, event.is_all_day ? `DTEND;VALUE=DATE:${toUtcStamp(event.end_at).slice(0, 8)}` : `DTEND:${toUtcStamp(event.end_at)}`, `SUMMARY:${escapeIcs(eventTitle(event, lang))}`, `DESCRIPTION:${escapeIcs(event.description || '')}`, `LOCATION:${escapeIcs(event.location || '')}`, 'END:VEVENT', 'END:VCALENDAR'].join('\r\n');
...
export function eventTypeColor(type: string) {
  switch (type) {
    case 'holiday': return 'calendar-type--holiday';
    case 'exam': return 'calendar-type--exam';
    case 'meeting': return 'calendar-type--meeting';
    case 'excursion': return 'calendar-type--excursion';
    case 'ceremony': return 'calendar-type--ceremony';
    default: return 'calendar-type--custom';
  }
}
