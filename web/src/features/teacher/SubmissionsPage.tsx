...
import { useGradeSubmission, useTeacherSubmissions } from './useTeacher';
import type { SubmissionItem } from './teacher.service';
              ...
              <tbody>
                {submissions.map((submission) => (
                  <tr key={submission.id}>
                    ...
                    <td><span className={`status-badge status-${submission.status}`}>{submission.status}</span></td>
                    <td>{submission.grade ? `${submission.grade.score}/${submission.assignment_total_points}` : '—'}</td>
                    <td style={{ fontSize: 13, color: 'var(--color-text-secondary)' }}>
                      {submission.submitted_at ? new Date(submission.submitted_at).toLocaleString() : '—'}
                    </td>
                    <td>
                      {(submission.status === 'submitted' || submission.status === 'graded') && (
                        <button className="btn btn-primary btn-sm" onClick={() => openGrading(submission)}>
        ...
        <div className="card" style={{ marginTop: 20, maxWidth: 500 }}>
          ...
          <form onSubmit={(event) => void handleGrade(event, gradingId)}>
            ...
            <label className="checkbox-label" style={{ marginBottom: 12 }}>
              <input type="checkbox" checked={gradePublish} onChange={(e) => setGradePublish(e.target.checked)} />
              {t('teacher.submissions.publishGrade')}
            </label>
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn btn-primary" type="submit" disabled={gradeSubmissionMutation.isPending}>
                {gradeSubmissionMutation.isPending ? t('app.loading') : t('app.save')}
              </button>
              <button className="btn btn-secondary" type="button" onClick={() => setGradingId(null)}>
                {t('app.cancel')}
              </button>
            </div>
          </form>
