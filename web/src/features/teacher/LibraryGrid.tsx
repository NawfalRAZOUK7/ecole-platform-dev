...
import { useAssignableClasses, useAssignContent, useSubmitContentForReview, useTeacherContentLibrary } from './useTeacher';
import { AssignContentModal } from './AssignContentModal';
import { ContentCard } from './ContentCard';
import { ContentFilters } from './ContentFilters';
import type { ClassOption, ContentItem } from './teacher.service';
...
export function LibraryGrid() {
  ...
  const items: ContentItem[] = useMemo(() => contentQuery.data?.pages.flatMap((page) => page.data) ?? [], [contentQuery.data]);
  const classes: ClassOption[] = classesQuery.data ?? [];
  const dismissibleError = useDismissibleError(useMemo(() => toBannerError(contentQuery.error ?? classesQuery.error ?? assignMutation.error ?? reviewMutation.error, t('app.error')), [assignMutation.error, classesQuery.error, contentQuery.error, reviewMutation.error, t]));

  async function handleAssign(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!assignItem || !assignClassId) return;
    await assignMutation.mutateAsync({ content_item_id: assignItem.id, class_id: assignClassId, notes: assignNotes.trim() || null });
    setAssignItem(null);
    setAssignClassId('');
    setAssignNotes('');
  }
  ...
  return (
    <>
      <ErrorBanner error={dismissibleError.error} onDismiss={dismissibleError.dismiss} onRetry={() => void contentQuery.refetch()} />
      <ContentFilters
      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {items.map((item) => (
            <ContentCard key={item.id} item={item} reviewPending={reviewMutation.isPending} onAssign={(content) => { setAssignItem(content); setAssignClassId(''); setAssignNotes(''); }} onSubmitForReview={(contentId) => void handleSubmitForReview(contentId)} />
          ))}
        </div>
      )}
