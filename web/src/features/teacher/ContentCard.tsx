...
import type { ContentCardProps } from './content-library.types';

export function ContentCard({ item, reviewPending, onAssign, onSubmitForReview }: ContentCardProps) {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button className="btn btn-primary" style={{ fontSize: 12, padding: '4px 10px' }} onClick={() => onAssign(item)}>
          {t('teacherContent.assignToClass')}
        </button>
        {item.school_id && (
          <button className="btn btn-secondary" style={{ fontSize: 12, padding: '4px 10px' }} onClick={() => onSubmitForReview(item.id)} disabled={reviewPending}>
