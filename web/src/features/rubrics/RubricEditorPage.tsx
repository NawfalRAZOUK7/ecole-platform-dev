...
import { useCreateRubric, useRubric, useUpdateRubric } from './useRubrics';
import type { CreateRubricPayload, RubricLevel } from './rubrics.types';
...
function toPayload(title: string, subject: string, description: string, criteria: EditCriterion[]): CreateRubricPayload {
  return {
    title,
...
export function RubricEditorPage() {
  ...
  useEffect(() => {
    if (rubricQuery.data) {
      ...
      setCriteria(r.criteria.map((c) => ({
        ...
        name: c.name,
        weight: c.weight,
        levels: c.levels.map((l) => ({ id: l.id, label: l.label, score: l.score, description: l.description })),
      })));
    }
  }, [rubricQuery.data]);
  ...
  function updateLevel(criterionIndex: number, levelIndex: number, field: keyof EditLevel, value: string | number) {
    ...
    <div className="page">
      <div className="page-header page-header--split">
        ...
        <div className="page-actions">
          ...
          {!isNew && (
            <button type="button" className="btn btn-secondary" onClick={() => navigate(`/rubrics/${id}/grade`)}>
              {t('rubrics.grade')}
            </button>
          )}
          <button type="button" className="btn btn-primary" disabled={isSaving || !title.trim()} onClick={() => void handleSave()}>
            {isSaving ? t('app.loading') : saved ? t('app.saved') : t('app.save')}
          </button>
        </div>
      </div>

      <ErrorBanner error={error ?? (rubricQuery.error instanceof Error ? rubricQuery.error.message : null)} onDismiss={() => setError(null)} />
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        <div className="form-field" style={{ flex: 1, minWidth: 140 }}>
          <label>{t('rubrics.cols.subject')} <span style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>({t('app.optional')})</span></label>
          <input className="input" value={subject} onChange={(e) => setSubject(e.target.value)} />
        </div>
        <div className="form-field" style={{ flex: 2, minWidth: 200 }}>
          <label>{t('rubrics.cols.description')} <span style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>({t('app.optional')})</span></label>
              ...
              {(criteria[0]?.levels ?? []).map((l, li) => (
                <th key={li} style={TH_STYLE}>
                  <input
                    ...
                    value={l.label}
                    onChange={(e) => criteria.forEach((_, ci) => updateLevel(ci, li, 'label', e.target.value))}
                  />
                  <input
                    type="number"
                    className="input"
                    ...
                    value={l.score}
                    onChange={(e) => criteria.forEach((_, ci) => updateLevel(ci, li, 'score', Number(e.target.value)))}
                  />
                  {levelCount > 2 && (
                    <button type="button" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: 'var(--color-danger)' }} onClick={() => criteria.forEach((_, ci) => removeLevel(ci, li))}>✕</button>
                  )}
                </th>
              ))}
              <th style={TH_STYLE}>
                <button type="button" className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: 12 }} onClick={() => criteria.forEach((_, ci) => addLevel(ci))}>
                  + {t('rubrics.addLevel')}
                </button>
              </th>
          ...
          <tbody>
            {criteria.map((c, ci) => (
              <tr key={ci} style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <td style={TD_STYLE}>
                  {criteria.length > 1 && (
                    <button type="button" className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: 12 }} onClick={() => removeCriterion(ci)}>✕</button>
                  )}
                </td>
              </tr>
            ))}
      ...
      <button type="button" className="btn btn-secondary" style={{ marginTop: 12 }} onClick={addCriterion}>
