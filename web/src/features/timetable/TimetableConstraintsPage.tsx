...
import type { RoomConstraint, TeacherAvailability, TimetableConstraints } from './timetable.service';
    ...
    <div className="page">
      <div className="page-header page-header--split">
        ...
        <div className="page-actions">
          <button type="button" className="btn btn-secondary" onClick={() => navigate('/timetable/generate')}>
      ...
      <ErrorBanner
        error={error ?? (constraintsQuery.error instanceof Error ? constraintsQuery.error.message : null)}
      ...
      <form onSubmit={handleSave}>
        ...
        <section style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <button type="button" className="btn btn-secondary" style={{ padding: '4px 10px' }} onClick={() => setAvailability((p) => [...p, { ...EMPTY_AVAIL }])}>
              + {t('timetable.constraints.addAvailability')}
            </button>
          </div>
          {availability.length === 0 && (
            <p style={{ color: 'var(--color-text-secondary)', fontSize: 13 }}>{t('timetable.constraints.noAvailability')}</p>
          )}
          {availability.map((a, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-end', marginBottom: 8, flexWrap: 'wrap' }}>
              ...
              <div className="form-field" style={{ minWidth: 130 }}>
                ...
                <select className="input" value={a.day_of_week} onChange={(e) => updateAvail(i, 'day_of_week', Number(e.target.value))}>
                  {DAYS.map((d) => <option key={d.value} value={d.value}>{d.label}</option>)}
                </select>
              </div>
              <div className="form-field" style={{ minWidth: 110 }}>
                {i === 0 && <label>{t('timetable.constraints.from')}</label>}
                <input type="time" className="input" value={a.available_from} onChange={(e) => updateAvail(i, 'available_from', e.target.value)} />
              </div>
              <div className="form-field" style={{ minWidth: 110 }}>
                {i === 0 && <label>{t('timetable.constraints.until')}</label>}
                <input type="time" className="input" value={a.available_until} onChange={(e) => updateAvail(i, 'available_until', e.target.value)} />
              </div>
              <button type="button" className="btn btn-secondary" style={{ padding: '6px 10px', marginBottom: 0 }} onClick={() => setAvailability((p) => p.filter((_, j) => j !== i))}>✕</button>
            </div>
          ))}
        </section>
        ...
        <section style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <button type="button" className="btn btn-secondary" style={{ padding: '4px 10px' }} onClick={() => setRooms((p) => [...p, { ...EMPTY_ROOM }])}>
              + {t('timetable.constraints.addRoom')}
            </button>
          </div>
          {rooms.length === 0 && (
            <p style={{ color: 'var(--color-text-secondary)', fontSize: 13 }}>{t('timetable.constraints.noRooms')}</p>
          )}
          {rooms.map((r, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-end', marginBottom: 8 }}>
              <div className="form-field" style={{ flex: 2 }}>
                ...
                <input className="input" placeholder={t('timetable.constraints.roomNamePlaceholder')} value={r.room_name} onChange={(e) => updateRoom(i, 'room_name', e.target.value)} />
              </div>
              <div className="form-field" style={{ minWidth: 110 }}>
                {i === 0 && <label>{t('timetable.constraints.capacity')}</label>}
                <input type="number" className="input" min={1} value={r.capacity} onChange={(e) => updateRoom(i, 'capacity', Number(e.target.value))} />
              </div>
              <button type="button" className="btn btn-secondary" style={{ padding: '6px 10px', marginBottom: 0 }} onClick={() => setRooms((p) => p.filter((_, j) => j !== i))}>✕</button>
            </div>
          ))}
        </section>

        <div style={{ display: 'flex', gap: 8 }}>
          <button type="submit" className="btn btn-primary" disabled={saveMutation.isPending || !academicYearId.trim()}>
