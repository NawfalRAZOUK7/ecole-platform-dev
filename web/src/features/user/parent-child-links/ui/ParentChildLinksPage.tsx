...
import type { ParentChildLinkRow, UserItem } from '@/features/admin/api/admin.api';
...
export function ParentChildLinksPage() {
  ...
  const dismissibleError = useDismissibleError(
    useMemo(
      () =>
        toBannerError(
          linksQuery.error ?? createLinkMutation.error ?? revokeLinkMutation.error,
          t('app.error'),
        ),
    ...
    <div className="page">
      ...
      >
        <h1 className="page-title" style={{ marginBottom: 0 }}>
          {t('admin.parentChildLinks.title')}
        </h1>
      ...
      {showCreate && (
        <div className="card" style={{ marginBottom: 20 }}>
          <h3 style={{ marginBottom: 12, fontSize: 15, fontWeight: 600 }}>
            {t('admin.parentChildLinks.createNew')}
          </h3>
            ...
            {selectedParent ? (
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ width: '100%' }}
                />
                {parentSearchResults.isFetching && (
            ...
            {selectedStudent ? (
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ width: '100%' }}
                />
                {studentSearchResults.isFetching && (
                  <span style={{ position: 'absolute', right: 8, top: 8, fontSize: 12 }}>
                    {t('app.loading')}
                  </span>
                )}
                    ...
                    <td>
                      <span
                        ...
                        style={{
                          ...
                          borderColor: getStatusColor(link.status),
                        }}
                      >
                        {t(
                          `admin.parentChildLinks.status${link.status.charAt(0).toUpperCase() + link.status.slice(1)}`,
                          link.status,
                        )}
                      </span>
                    </td>
