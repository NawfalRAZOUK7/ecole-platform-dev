...
import { useRevokeSession, useSessions } from '../model/useProfile';
import type { SessionItem } from '../api/profile.api';
              ...
              >
                <div>
                  ...
                  >
                    ...
                    <span>
                      {t('sessions.created')}: {new Date(session.created_at).toLocaleString()}
                    </span>
                    {session.last_active && (
                      <span>
                        {t('sessions.lastActive')}: {new Date(session.last_active).toLocaleString()}
                      </span>
                    )}
                  </div>
                  {session.user_agent && (
                    ...
                    >
                      {session.user_agent.length > 80
                        ? `${session.user_agent.slice(0, 80)}...`
                        : session.user_agent}
                    </div>
                  )}
                </div>
