...
import type { Invitation, UserItem } from '@/features/admin/api/admin.api';
...
export function InvitationsPage() {
  ...
  async function handleCreate() {
    ...
    const response = await createInvitationMutation.mutateAsync({
      ...
      target_student_id:
        createRole === 'PAR' && selectedTargetStudent ? selectedTargetStudent.id : undefined,
    });
  ...
  if (invitationsQuery.isLoading) {
    return (
      <div className="page">
        <h1 className="page-title">{t('admin.invitations.title')}</h1>
        <ListSkeleton />
      </div>
    );
  }
          ...
          {createRole === 'PAR' && (
            <div style={{ marginTop: 12 }}>
              ...
              {selectedTargetStudent ? (
                ...
                >
                  <span>
                    {selectedTargetStudent.full_name} ({selectedTargetStudent.email})
                  </span>
                  <button
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  {targetStudentQuery.isFetching && (
                    <span style={{ position: 'absolute', right: 8, top: 8, fontSize: 12 }}>
                      {t('app.loading')}
                    </span>
                  )}
                  ...
                  <tr key={item.id}>
                    <td>
                      <span className="role-badge">
                        {t(`roles.${item.role_target}`, item.role_target)}
                      </span>
                    </td>
                    <td>
                      <span
                      ...
                      >
                        {t(
                          `admin.invitations.status${item.status.charAt(0).toUpperCase() + item.status.slice(1)}`,
                          item.status,
                        )}
                      </span>
                    </td>
