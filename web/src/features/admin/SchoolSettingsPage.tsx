    ...
    <div className="page">
      ...
      <form className="card settings-card" style={{ maxWidth: 720 }} onSubmit={handleSubmit}>
        <div className="form-field">
          ...
          <input id="school-name" className="input" value={form.name} onChange={(event) => updateField('name', event.target.value)} required />
        </div>
        <div className="form-field">
          <label htmlFor="school-address">{t('admin.settings.address')}</label>
          <input id="school-address" className="input" value={form.address} onChange={(event) => updateField('address', event.target.value)} />
        </div>
        <div className="form-field">
          <label htmlFor="school-city">{t('admin.settings.city')}</label>
          <input id="school-city" className="input" value={form.city} onChange={(event) => updateField('city', event.target.value)} />
        </div>
        <div className="form-field">
          <label htmlFor="school-phone">{t('admin.settings.phone')}</label>
          <input id="school-phone" className="input" value={form.phone} onChange={(event) => updateField('phone', event.target.value)} />
        </div>

        <div style={{ display: 'grid', gap: 8, margin: '12px 0 20px', color: 'var(--color-text-secondary)', fontSize: 13 }}>
          <span>{t('admin.settings.schoolId')}: {user.school_id}</span>
          <span>{t('admin.settings.schoolCode')}: {schoolQuery.data?.code || '—'}</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          {saved ? <span className="save-state save-state--success">{t('admin.settings.saved')}</span> : null}
        </div>
      </form>
    </div>
  );
