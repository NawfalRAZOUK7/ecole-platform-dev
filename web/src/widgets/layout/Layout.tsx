...
import { useProfileData } from '@/features/user/profile';
import { adminQueryKeys } from '@/features/admin/model/useAdmin';
import { adminService } from '@/features/admin/api/admin.api';
import { invoicesQueryKeys } from '@/features/billing/invoices/model/useInvoices';
import { invoicesService } from '@/features/billing/invoices/api/invoices.api';
import { notificationQueryKeys } from '@/features/communication/notifications/model/useNotifications';
import { notificationsService } from '@/features/communication/notifications/api/notifications.api';
import { useFeedUnreadSummary } from '@/features/content/feed/model/useFeed';
import { useSyncDevices, useSyncHealth, useSyncStatus } from '@/features/sync/model/useSync';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: '/admin/programs', labelKey: 'nav.adminPrograms', icon: '📚', roles: ['ADM', 'DIR'] },
  { to: '/admin/enrollments', labelKey: 'nav.adminEnrollments', icon: '🎒', roles: ['ADM', 'DIR'] },
  {
    to: '/admin/program-equivalences',
    labelKey: 'nav.adminEquivalences',
    icon: '🔗',
    roles: ['ADM', 'DIR'],
  },
  {
    to: '/admin/eligibility-rules',
    labelKey: 'nav.adminEligibilityRules',
    icon: '📜',
    roles: ['ADM', 'DIR'],
  },
    ...
    (to: string) => {
      if (to === '/admin') {
        ...
        void queryClient.prefetchInfiniteQuery({
          ...
          initialPageParam: undefined as string | undefined,
          getNextPageParam: (lastPage: Awaited<ReturnType<typeof notificationsService.list>>) =>
            lastPage.meta.has_more ? (lastPage.meta.next_cursor ?? undefined) : undefined,
          staleTime: 5 * 60 * 1000,
        });
