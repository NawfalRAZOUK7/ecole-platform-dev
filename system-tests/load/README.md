...
```bash
# Verify API is alive
BASE_URL=http://localhost:8010/api/v1 k6 run smoke/healthcheck.js

# Run baseline login test
BASE_URL=http://localhost:8010/api/v1 k6 run baseline/01_logins.js

# Run all baselines
for f in baseline/*.js; do BASE_URL=http://localhost:8010/api/v1 k6 run "$f"; done
```
...
The shared k6 config refuses the normal dev database target at
`localhost:8000` unless `K6_ALLOW_DEV_DB=1` is set intentionally.
