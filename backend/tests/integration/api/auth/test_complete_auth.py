...
class TestRecovery:
    ...
    @pytest.mark.asyncio
    async def test_recovery_request_with_testmail(self, client: httpx.AsyncClient):
        """Recovery request with a real Testmail inbox (Student Pack E2E).

        Requires: TESTMAIL_API_KEY set and the dev server configured with
        an internet-facing SMTP relay (not localhost:1025).
        """
        import os

        testmail_api_key = os.getenv("TESTMAIL_API_KEY", "")
        if not testmail_api_key:
            pytest.skip("TESTMAIL_API_KEY not configured")

        from tests.utils.testmail import fresh_email, wait_email

        tag = f"recovery-{uuid.uuid4().hex[:8]}"
        to = fresh_email(tag)

        resp = await client.post(
            "/recovery/request",
            json={"email": to, "school_id": SCHOOL_ID},
        )
        assert resp.status_code == 200

        # Poll Testmail — only works if the dev server SMTP reaches the internet.
        try:
            email = wait_email(tag, timeout=30)
            assert email["to"] == to
            assert "code" in email.get("subject", "").lower()
        except TimeoutError:
            pytest.skip(
                "Email not received in Testmail — "
                "check that the dev server SMTP is configured with a real relay"
            )
