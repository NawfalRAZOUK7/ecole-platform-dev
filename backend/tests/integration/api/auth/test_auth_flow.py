...
# ---------------------------------------------------------------------------
# Testmail — E2E email verification (Student Pack)
# ---------------------------------------------------------------------------
class TestInvitationWithTestmail:
    """Use Testmail disposable inboxes for invitation/registration E2E."""

    @pytest.mark.asyncio
    async def test_register_with_testmail_address(
        self, client: httpx.AsyncClient, admin_token: str
    ):
        """Register a user with a real Testmail inbox address."""
        import os
        ...
        from tests.utils.testmail import fresh_email

        # Create invite as admin
        invite_resp = await client.post(
            "/invites/create",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={"role_target": "STD", "expires_in_hours": 24},
        )
        assert invite_resp.status_code == 201
        code = invite_resp.json()["data"]["code"]

        # Register with a Testmail address
        tag = f"invite-{uuid.uuid4().hex[:8]}"
        ...
        reg_resp = await client.post(
            ...
            json={
                "code": code,
                "email": email,
                "full_name": "Testmail Student",
                "password": STRONG_PASSWORD,
            },
        )
        assert reg_resp.status_code == 201
        assert reg_resp.json()["data"]["user_id"] is not None
