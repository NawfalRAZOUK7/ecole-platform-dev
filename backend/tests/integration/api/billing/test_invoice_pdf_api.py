...
from tests.integration.api.helpers import INVOICE_ID, auth_header
...
class TestInvoicePdfApi:
    ...
    async def test_admin_can_request_invoice_pdf_ar(self, client, admin_token):
        ...
        data = response.json()["data"]
        assert data["type"] == "invoice_pdf"
        # In dev/test environments without wkhtmltopdf, generation may fail
        # The API contract is still valid: 201 + job payload
