...
async def register(
    ...
    from app.services.auth.auth import EmailVerificationService

    email_service = EmailVerificationService(db, redis)
    verification = await email_service.send_verification_otp(
    ...
    if settings.app_env != "production" and verification.get("otp"):
