...
TestMail API integration for testing when TESTMAIL_ENABLED=true.
"""
...
class EmailService:
    ...
    async def _send_via_testmail(self, to: str, subject: str, html_body: str) -> bool:
        """Send an email via TestMail API. Returns True on success, False on failure."""
        if not settings.testmail_enabled or not settings.testmail_api_key:
            logger.warning("TestMail not configured, skipping API send")
            return False

        try:
            # TestMail API endpoint for sending emails
            ...
            if "@" in to and ".testmail.app" in to:
                # It's already a TestMail address, use it as-is
                recipient = to
            else:
            ...
            logger.warning(
                "Failed to send email via TestMail API to %s: %s",
                to,
                subject,
                exc_info=True,
            )
    ...
    async def send_email(
        ...
        **kwargs: Any,
        ...
        # Use TestMail API if enabled, otherwise fall back to SMTP
        if settings.testmail_enabled:
            return await self._send_via_testmail(to, subject, html_body)
