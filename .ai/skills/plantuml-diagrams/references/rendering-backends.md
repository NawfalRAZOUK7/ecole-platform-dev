...
- **Don't quietly fall back.** If the chosen backend is unavailable, do *not*
  silently switch to another. Tell the user which backend failed and what the
  alternatives are, then proceed only with a sensible default or their go-ahead.
- **Report the path.** State which backend rendered and whether the source left
  the machine — e.g. *"rendered via public Kroki (source uploaded to kroki.io)"*
  vs *"rendered via local Kroki (stayed on your machine)."*
