...
Kroki renders PlantUML server-side with a recent PlantUML + Graphviz. Two hard limits:
...
things to drop when degrading.

## Failure-degradation ladder

If re-rendering after a targeted line fix still returns `400` (or the layout is
unusable), simplify in this order, re-running the export after each step. Stop
...
5. **Reduce edges** — cut redundant relationships.
6. **Switch engine** — render the core idea in a simpler diagram type rather
   than forcing the current one.
...
- Smart quotes / non-breaking spaces / tabs pasted from a doc — retype the line.
- Unbalanced `{ }` in `package` / `rectangle` / class bodies.

### Sequence
- `alt` / `opt` / `loop` / `par` / `group` each need a matching `end`.
- Reversed arrow: use `A -> B` for left-to-right intent.
- Participants render in first-mention order — declare `participant` at the top.

### Class
- Relationship glyphs: `<|--` inheritance, `*--` composition, `o--` aggregation,
  `..|>` realization. Mixing dashes/heads silently changes meaning.
- Multiplicities must be quoted: `User "1" --> "*" Order`.

### Activity
- Do not mix the legacy `(*)` syntax with the modern `start` / `:action;` / `stop` syntax.
- Every `if (…) then` needs `endif`; every `repeat` needs `repeat while`.

### State
- Use `[*]` for initial/final pseudo-states; `[ ]` or `()` will fail.
- Composite states need `state Name { … }` braces, not indentation.

### ER / entity
- Crow's-foot cardinality between entities: `user ||--o{ order`.
- Attribute block separator is `--` on its own line inside `entity { }`.

### Component
- Overlap/crowding is a layout symptom, not an error — add `together { }` or a
  direction hint before assuming the source is wrong.

### C4
- `400` with "cannot include" → remote `!includeurl`; switch to bundled `!include <C4/C4_Context>`.
