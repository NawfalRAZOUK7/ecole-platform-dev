...
1. **Read** the relevant files — never infer structure from file names alone.
2. **Extract** entities + relationships into a short list and sanity-check it against the code before drawing.
3. **Emit** `.puml` from that list, preserving the real identifiers. Keep member signatures concise.
4. **Scope large inputs** — one module/package per diagram. Split rather than cram.
